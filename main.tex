...
We compare MPEC with their kernel-smoothed frequency estimator with a look-up table, the most frequently used method in empirical work.\footnote{The detailed construction is described in Appendix. For the kernel-smoothed frequency simulator, we need a tuning scaling vector.} 
...
All estimations start from a vector of zeros.
Our replication Julia code is available on the authors' github.
...
\begin{table}[!htbp]
  ...
  \end{center}
  \footnotesize
  Note: We write Julia code for both estimations for fair comparison. The benchmark results correspond with Column (4) in Table B1: Monte Carlo Simulation Results of \cite{ursu2023sequential}.
\end{table} 
...
It shows that our MPEC estimator achieves a slightly positive biased and larger RMSE performance than the benchmark even without any ad hoc imposed assumption in a look-up table. 
The computational time is longer than the benchmark.
