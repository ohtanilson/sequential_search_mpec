...
\textcolor{blue}{
Based on Monte Carlo simulations, MPEC shows comparable or better performance for the sequential search model relative to the nested fixed point approaches and the advantage carries over. 
The main drawbacks of MPEC are the use of PC memory and computational time.
We conclude that MPEC is a better alternative among the current approaches when the approximation and estimation errors and ad hoc look-up table are unknown to researchers.
}
...
We compare MPEC with their kernel-smoothed frequency estimator with a look-up table, the most frequently used method in empirical work. 
...
It shows that our MPEC estimator achieves better performance than the first approach without any ad hoc imposed assumption in a look-up table. 
\begin{itemize}
    \item Bias and RMSE
    \item However, computational time is longer than the first approach.
    \item olumn (4) in Table B1: Monte Carlo Simulation Results.
\end{itemize}
...
\begin{table}[!htbp]
  \begin{center}
      ...
      \subfloat[MPEC (kernel) $\sigma=20$]{\input{figuretable/results_MPEC_1_bias_rmse}}
      \subfloat[Benchmark (kernel)]{\input{figuretable/results_MPEC_1_bias_rmse}}
  \end{center}
  \footnotesize
  Note: 
\end{table} 
...
Following \cite{ursu2023sequential}, weconstruct individual likelihood $L_{i}(\theta,(z_{ij}(\theta))_{j\in \mathcal{J}})$ as follows. 
\begin{align*}
