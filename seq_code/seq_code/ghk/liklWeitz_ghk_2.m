function llk = liklWeitz_ghk_2(param, dat, D, nalt, seed)
...
%choices
tran=dat(:,end);
tran_ranking = sum(reshape(tran, N_prod, N_cons).*repmat(1:N_prod, N_cons, 1)', 1)';
searched=dat(:,end-1);

searched_amt = sum(reshape(searched, N_prod, N_cons), 1)';
...
for con = 1:N_cons
    ...
    if tr == 1 && H > 1
        ...
        rng(con*seed*1122);
        mu_i(H, :) = norminv(rand(1, size(b_iH, 2)).*(1 - normcdf(b_iH)) + normcdf(b_iH));
        ...
        l_stop = ones(1, D);
        for l = S_bar
            l_stop = l_stop.*normcdf(mu_i(1, :) - xb_con_prod(l, con) - mc(l, con));
        end
        % Continuation rule
        l_cont = 1 - normcdf(b_iH);
        % Selection rule
        l_selection = ones(1, D);
        for h = [H - 1:-1:2]
            b_ih = xb_con_prod(h + 1, con) + mu_i(h + 1, :) - xb_con_prod(h, con);
            l_selection = l_selection.*(1 - normcdf(b_ih));
        end
        % Choice rule
        l_choice = ones(1, D);
        for h = [2:1:H]
            l_choice = l_choice.*normcdf(mu_i(1, :) - mu_i(h, :) - xb_con_prod(h, con));
        end
    ...
    elseif tr == H && H > 1
        ...
        % Step 2: Sampling mu_iH
        b_iH = max([(mu_i(1, :) - xb_con_prod(H, con) - mc(H, con)); (-xb_con_prod(H, con) + xb_con_prod(S_bar, con) + mu_i(S_bar, :))], [], 1);
        rng(seed*con*1122);
        mu_i(H, :) = norminv(rand(1, size(b_iH, 2)).*(1 - normcdf(b_iH)) + normcdf(b_iH));
        % Step 3: Sampling mu_i(1:(H - 1))
        for h = [H - 1:-1:2]
        ...
        for h = 2:(H - 1)
            ...
            rng(seed*con*3344 + h);
            eps_i(h, :) = norminv(rand(1, size(b_eps_ih, 2)).*(normcdf(b_eps_ih)));
        end
        % Stopping rule
        l_stop = 1 - normcdf(max([mu_i(1, :); (xb_con_prod(S_bar, con) + mu_i(S_bar, :) + mc(S_bar, con)); (mu_i(2:(H - 1), :) + xb_con_prod(2:(H - 1), con) + eps_i(2:(H - 1), :))], [], 1)- xb_con_prod(H, con) - mu_i(H, :));
        % Continuation rule
        l_cont = 1 - normcdf(b_iH);
        % Selection rule
        l_selection = ones(1, D);
        ...
        l_choice = ones(1, D);
        for h = 2:(H - 1)
            l_choice = l_choice.*normcdf(xb_con_prod(H, con) + mu_i(H, :) + mc(H, con) - xb_con_prod(h, con) - mu_i(h, :));
        end
    ...
    elseif (tr > 1) && (tr < H)
        ...
        b_eps_lower = max([(xb_con_prod(S_bar, con) + mu_i(S_bar, :) + mc(S_bar, con)); mu_i(1, :)], [], 1) - xb_con_prod(tr, con) - mu_i(tr, :);
        rng(seed*con*3344);
        eps_i(tr, :) = norminv(rand(1, size(b_eps_upper, 2)).*(normcdf(b_eps_upper) - normcdf(b_eps_lower)) + normcdf(b_eps_lower));
        % Stopping rule
        l_stop = normcdf(b_eps_upper) - normcdf(b_eps_lower);
        % Continuation rule
        l_cont = 1 - normcdf(b_iH);
        % Selection rule
        l_selection = ones(1, D);
        ...
        % Choice rule
        l_choice = ones(1, D);
        for h = setdiff(2:H, tr)
            l_choice = l_choice.*normcdf(xb_con_prod(tr, con) + mu_i(tr, :) + eps_i(tr, :) - xb_con_prod(h, con) - mu_i(h, :));
        end
